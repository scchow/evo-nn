...
def main():
    ...
    # probs = ["0.000000", "0.100000", "0.200000", "0.300000", "0.500000", "0.600000", "0.700000","0.800000", "0.900000", "1.000000"]
    probs = ["0.000000", "0.200000", "0.400000", "0.600000", "0.800000", "0.900000"]
    # probs = [10, 50]
    # baseResultsPath = os.path.join("build", "Results", date, "MultiNightBarQ","adaptive_"+adapt_type)

    # paths = map(lambda x: os.path.join(baseResultsPath, "temp_"+str(x),str(numAgents)+"_agents", "0_disabled"), probs)

    paths = map(lambda x: os.path.join("build", "Results", "fixed_probability", "MultiNightBarQ", "fixed_prob_learning", "prob_"+str(x), str(numAgents)+"_agents", "0_disabled"), probs)
    ...
    ax.legend([handles[idx] for idx in order],[labels[idx] for idx in order], loc='center left', bbox_to_anchor=(1, 0.5), title="Probability")
    plt.show()  
